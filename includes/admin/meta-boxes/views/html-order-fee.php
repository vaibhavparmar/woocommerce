...
<tr class="fee <?php echo esc_attr( ! empty( $class ) ? $class : '' ); ?>" data-order_item_id="<?php echo esc_attr( $item->get_id() ); ?>">
	...
	<td class="name">
		...
		<div class="edit" style="display: none;">
			<input type="text" placeholder="<?php esc_attr_e( 'Fee Name', 'woocommerce' ); ?>" name="order_item_name[<?php echo esc_attr( $item->get_id() ); ?>]" value="<?php echo esc_attr( $item->get_name() ); ?>" />
	...
	<td class="line_cost" width="1%">
		<div class="view">
			<?php
				echo wc_price( wc_round_tax_total( $item->get_total() ) );

				if ( $refunded = $order->get_total_refunded_for_item( $item->get_id(), 'fee' ) ) {
					echo '<small class="refunded">-' . wc_price( $refunded ) . '</small>';
				}
			?>
		</div>
		<div class="edit" style="display: none;">
			<input type="text" name="line_total[<?php echo esc_attr( $item->get_id() ); ?>]" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" value="<?php echo esc_attr( wc_format_localized_price( $item->get_total() ) ); ?>" class="line_total wc_input_price" />
		</div>
					...
					<td class="line_tax" width="1%">
						<div class="view">
							<?php
								...
								if ( $refunded = $order->get_tax_refunded_for_item( $item->get_id(), $tax_item_id, 'fee' ) ) {
									echo '<small class="refunded">-' . wc_price( $refunded ) . '</small>';
								}
							?>
						</div>
						<div class="edit" style="display: none;">
							<input type="text" name="line_tax[<?php echo esc_attr( $item->get_id() ); ?>][<?php echo esc_attr( $tax_item_id ); ?>]" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" value="<?php echo esc_attr( wc_format_localized_price( $tax_item_total ) ); ?>" class="line_tax wc_input_price" />
						</div>
						<div class="refund" style="display: none;">
